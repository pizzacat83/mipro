...
void erase_front(List* const list_p) {
    Node* oldhead_p = list_p->head;
    list_p->head = oldhead_p->next;
    if (list_p->tail == oldhead_p) {
        list_p->tail = NULL;
    } 
    delete_node(oldhead_p);
}
...
void erase_search_list_key(List* const list_p, HashKey k) {
    Node* prev_p = NULL;
    Node* res_p = NULL;
    FOREACH (*list_p, node_p) {
        if (cmp(node_p->value.k, k) == 0) {
            res_p = node_p;
            break;
        }
    ...
    if (res_p == list_p->tail) {
        list_p->tail = prev_p;
    }
    prev_p->next = res_p->next;
    delete_node(res_p);
}

void erase_search_hash(Hash hash, HashKey k) {
    erase_search_list_key(&hash.slots[key_to_index(hash, k)], k);
}
    ...
    while (true) {
        ...
        switch (q) {
            ...
            case 'd': {
                ...
                erase_search_hash(hash, k);
                free(k);
                break;
            }
            case 's': {
                HashKey k = read_new_string();
                HashValue* res_p = search_hash(hash, k);
                if (res_p == NULL) {
                    printf("Not found\n");
                } else {
                    printf("%s %s\n", k, *res_p);
